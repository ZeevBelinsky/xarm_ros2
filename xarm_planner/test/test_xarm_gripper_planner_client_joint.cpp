...
int main(int argc, char** argv)
{
	...
	while (rclcpp::ok())
	{
		...
		RCLCPP_INFO(node->get_logger(), "did the step - plan open");
		call_request(exec_plan_client_, exec_plan_req);
		RCLCPP_INFO(node->get_logger(), "did the step - exec open");
		rclcpp::sleep_for(std::chrono::seconds(3));

		joint_plan_req->target = tar_joint2;
		call_request(joint_plan_client_, joint_plan_req);
		RCLCPP_INFO(node->get_logger(), "did the step - plan close");
		call_request(exec_plan_client_, exec_plan_req);
		RCLCPP_INFO(node->get_logger(), "did the step - exec close");
		rclcpp::sleep_for(std::chrono::seconds(3));
	}
