...
double XArmPlanner::compute_path_cost(const trajectory_msgs::msg::JointTrajectory& jt) const {
	double ans = jt.points.size();
	RCLCPP_INFO(node_->get_logger(), "compute_path_cost: path cost=%lf", ans);
	return ans;
}
